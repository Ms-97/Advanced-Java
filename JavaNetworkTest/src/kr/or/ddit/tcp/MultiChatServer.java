...
public class MultiChatServer {
	...
	public void sendMessage(String msg) {
		...
		while(it.hasNext()) {
			try {
				...
				// 대화명에 해당하는 Socket객체에서 OutputStream객체 구하기
				DataOutputStream dos = 
						new DataOutputStream(
								clients.get(name).getOutputStream());
				dos.writeUTF(name); // 메시지 보내기
	...
	/**
	 * 서버에서 클라이언트로 메시지를 전솔할 Thread클래스를 Inner클래스로
	 * 정의한다. 
	...
	class ServerReceiver extends Thread {
		...
		public void run() {
			try {
				...
				// 이 finally영역이 실행된다는 의미는 클라이언트의 접속이 
				// 종료되었다는 의미이다.
