...
class LockAccount {
	...
	public void deposit(int money) {
		// Lock객체의 lock()가 동기화 시작이고, unlock()메소드가
		// 동기화의 끝을 나타낸다.
		// lock()으로 동기화를 설정한 곳에서는 반드시 unlock()메소드로
		// 해제해 주어야 한다.
		lock.lock(); //락 설정
	...
	// 출금하는 메소드(출금 성공: true, 출금 실패 : false 반환)
	public boolean withdraw(int money) {
		...
		try {
			...
			// try~catch 블럭을 사용할 경우에는 unlock() 호출은
			// finally 블럭에서 하도록 한다.
